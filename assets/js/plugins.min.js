...
if(config.submenuWidth!=="auto"){var submenuPotentialPosition=itemPosition+config.submenuWidth*widthMultiplier;if(window.innerWidth<submenuPotentialPosition){if(submenu&&submenu.classList){submenu.classList.add(config.submenuRightPositionClass);}}else{if(submenu&&submenu.classList){submenu.classList.add(config.submenuLeftPositionClass);}}}else{var submenuPotentialPosition=0;var submenuPosition=0;if(widthMultiplier===1){submenuPotentialPosition=itemPosition+submenu.clientWidth;}else{submenuPotentialPosition=itemPosition+this.clientWidth+submenu.clientWidth;}
...
if(submenu){submenu.setAttribute("aria-hidden",false);}});submenuParents[i].addEventListener("mouseleave",function(){var submenu=this.querySelector(config.submenuSelector);if(submenu){submenu.removeAttribute("style");submenu.setAttribute("aria-hidden",true);}});}}
...
Math.easeInOutQuad=function(t,b,c,d){t/=d/2;if(t<1)return(c/2)*t*t+b;t--;return(-c/2)*(t*(t-2)-1)+b;};function hasClass(el,className){if(el.classList)return el.classList.contains(className);else
...
if(classList.length>1)removeClass(el,classList.slice(1).join(" "));}})();(function(){var shareButton=document.querySelector(".post__share-button");var sharePopup=document.querySelector(".post__share-popup");if(shareButton){sharePopup.addEventListener("click",function(e){e.stopPropagation();});shareButton.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();sharePopup.classList.toggle("is-visible");});document.body.addEventListener("click",function(){sharePopup.classList.remove("is-visible");});}
...
var px=Math.floor(((screen.availWidth||1024)-Config.Width)/2),py=Math.floor(((screen.availHeight||700)-Config.Height)/2);var link_href=t.href?t.href:t.parentNode.href;var popup=window.open(link_href,"social","width="+
Config.Width+",height="+
Config.Height+",left="+
px+",top="+
py+",location=0,menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1");if(popup){popup.focus();if(e.preventDefault)e.preventDefault();e.returnValue=false;}
return!!popup;}})();